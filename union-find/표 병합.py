...
- union은 루트 노드를 상대 그룹에 연결해야만 의미가 있다. -> 트리 구조를 유지할 수 있게
- 루트 노드 판별에 자기 자신을 가리키는지를 확인하기 때문에
    - 로직에 자기 자신을 가리키게 원래대로 바꾸면서 find_parent를 하게 되면 끊어진 트리를 가져오게 된다.
    - 원래대로 바꾸는 로직을 스택에 저장해서 한번에 몰아할 것
- 부모 노드를 선택할 때, 딱히 기준은 없어도 된다.
'''
